...
namespace api {
    ...
    public:
        ...
        boost::asio::io_service & get_executor().context() {
            return m_ws.get_executor().context();
        }
    ...
    private:
        void do_write() {
            ...
            m_wq.pop_front();
            if (!m_wq.empty())
                m_writer.get_executor().context().post(std::bind(&this->do_write,this));
        }
    ...
    private:
        void do_read() {
            ...
            m_rq.pop_front();
            if (!m_rq.empty())
                m_reader.get_executor().context().post(std::bind(&this->do_read,this));
        }
